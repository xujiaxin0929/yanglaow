...
function selectD(p,c,d) {
	...
	$("#expressArea").val(expressArea);
	if(expressArea.indexOf('北京市')>=0||expressArea.indexOf('天津市')>=0||expressArea.indexOf('上海市')>=0||expressArea.indexOf('重庆市')>=0)
	{
		var citys=	expressArea.split('>');
		expressArea=citys[0]+'>'+citys[0]+'>'+citys[1]
	}
	$.callback(expressArea)
}

/*关闭省市区选项*/
function clockArea() {
	$("#areaMask").fadeOut();
	$("#areaLayer").animate({"bottom": "-300%"});
	intProvince();
}
...
$(function() {
	...
	/*初始化省份*/
	$.selectCity=function (pro,cty) {
		if(pro=='北京市'||pro=='天津市'||pro=='上海市'||pro=='重庆市')
		{
			for (var i=0; i<province.length; i++) {
				if(pro==province[i])
				{
					return [pro,cty,district[i][0][0]]
				}
			}
		}
		else
