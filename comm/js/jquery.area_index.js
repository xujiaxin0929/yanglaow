...
/*选择省份*/
function selectP(p) {
	if(p!=0&&p!=1&&p!=10&&p!=21)
	{
		areaCont = "";
		areaList.html("");
		for (var j=0; j<city[p].length; j++) {
			areaCont += '<li onClick=selectCityName(this)> ' + city[p][j] + '</li>';
		}
		areaList.html(areaCont);
		$("#areaBox").scrollTop(0);
		expressArea = province[p] +">";
		$("#backUp").attr("onClick", "intProvince();").show();
		$.cityName=expressArea;
	}
	else
	{
		$.cityName=province[p];
		$("#city").html(province[p]);
		clockArea(province[p])
	}
	
}
function selectCityName(cityname){
	$.cityName+=$(cityname).html().trim()
	$("#city").html($(cityname).html());
	clockArea($(cityname).html());
}
/*选择城市*/
